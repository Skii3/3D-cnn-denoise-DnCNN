...
def kernelshow(g,n_kernel,sess,epoch,bn_select):
    ...
    for k in range(n_kernel):
        temp = sess.run(g.get_tensor_by_name(prefix+'conv' + str(k + 1) + '/kernel:0'))
        min = np.min(temp)
        if min < min_all:
            min_all = min
    ...
    if bn_select != 0:
        ...
        for k in range(2,n_kernel,1):
            beta = sess.run(g.get_tensor_by_name(prefix+'bn' + str(k) + '/beta:0'))
            gamma = sess.run(g.get_tensor_by_name(prefix+'bn' + str(k) + '/gamma:0'))
